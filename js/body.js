...
if (getUrlParameter("token") && getUrlParameter("method")) {
  Swal.fire({
    title: "Loading...",
    icon: "info",
    ...
    timer: 1000,
    willClose: () => {
      SendToHewkawArApplicationService();
    },
  });
} else if (getUrlParameter("token")) {
  Swal.fire({
    title: "Loading...",
    icon: "info",
    showConfirmButton: false,
...
function getSession(month, year, popup = false) {
  ...
  axios(`https://api.hewkawar.xyz/app/bank/session?session_id=${session_id}`)
    .then((response) => {
...
function getBalance(session_id, month, year) {
  if (month && year) {
    axios(
      `https://api.hewkawar.xyz/app/bank/transition?session_id=${session_id}&month=${month}&year=${year}`
    ).then((response) => {
      const loadScreen = document.getElementById("loadScreen");
      const loader = document.getElementById("loader");
      loadScreen.style.display = "none";
      loader.style.display = "none";
      balance_value.innerText = `${formatNumberWithCommas(
        response.data.account.balance
      )} THB`;
      balance_pua_value.innerText = `${formatNumberWithCommas(
        response.data.account.balance_chip
      )} PUA`;
      deposit_value.innerText = `${formatNumberWithCommas(
        response.data.account.deposit
      )} THB`;
      withdraw_value.innerText = `${formatNumberWithCommas(
        response.data.account.withdraw
      )} THB`;
      balance_all_value.innerHTML = `${formatNumberWithCommas(
        response.data.account.balance + response.data.account.balance_chip
      )} THB`;
    });
  } else {
    axios(
      `https://api.hewkawar.xyz/app/bank/balance?session_id=${session_id}`
    ).then((response) => {
      const loadScreen = document.getElementById("loadScreen");
      const loader = document.getElementById("loader");
      loadScreen.style.display = "none";
...
function getHewkawArLoginDetail() {
  axios
    .get(`https://auth-api.hewkawar.xyz/token/${getUrlParameter("token")}`)
    .then((response) => {
      axios
        .post("https://api.hewkawar.xyz/app/bank/session", {
...
function SendToHewkawArApplicationService() {
  const token = getUrlParameter("token");

  window.location.href = `bankbyhewkawar://callback?token=${token}`;
}

function HewkawArLogin(token) {
  if (navigator.userAgent.startsWith("HewkawArApplicationService")) {
    window.location.href = `https://auth.hewkawar.xyz/logout?redirect_uri=${window.location.href}&method=HewkawArApplicationService`;
  } else {
...
function deposit() {
  ...
  Swal.fire({
    ...
    preConfirm: async (deposit) => {
      await axios
        .post("https://api.hewkawar.xyz/app/bank/deposit", {
        ...
        preConfirm: async (value) => {
          try {
            const response = await axios.post(
              "https://api.hewkawar.xyz/app/bank/withdraw",
              {
                session_id: session_id,
                amount: result.value,
                currency: value,
...
function convertTHB() {
  ...
  Swal.fire({
    ...
    preConfirm: async (thb) => {
      await axios
        .post("https://api.hewkawar.xyz/app/bank/convert/thb", {
          session_id: session_id,
          amount: thb,
        })
        .then((response) => {
...
function convertPUA() {
  ...
  Swal.fire({
    ...
    preConfirm: async (thb) => {
      await axios
        .post("https://api.hewkawar.xyz/app/bank/convert/pua", {
          session_id: session_id,
          amount: thb,
        })
        .then((response) => {
...
function populateMonthYearDropdown() {
  ...
  var defaultOption = document.querySelector(
    "#monthYearSelect option[value='" + defaultValue + "']"
  );
  if (defaultOption) {
